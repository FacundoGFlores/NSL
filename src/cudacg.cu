 ...
 */
void cudacgFillZeros(float *dst, const int N)
{
	for(int i = 0; i < N; i++)
		dst[i] = 0;
}
 ...
 */
void cudacgGetArray(float *dst, const vector<float> &vec)
{
 ...
 */
long cudaConjugate_Gradient(const linearMatrix &Matrix_A, const linearMatrix &Vector_B, linearMatrix &Vector_X,
						const float e, const int MaxIter)
{
	...
	//Set vectors
	cudacgGetArray(mA, Matrix_A.Get_Vector());
	cudacgGetArray(vB, Vector_B.Get_Vector());
	cudacgGetArray(vX, Vector_X.Get_Vector());
	...
	while(myerror > new_e  && k < MaxIter)
	{
		...
		else
		{
			...
			//helper
			cudaMemcpy(dev_r_tmp, dev_r, N * sizeof(float), cudaMemcpyDeviceToDevice);
