...
Version     : 	0.1
Description : 	This is the pre-conditioner conjugate gradient method using an iterative way.
License     :
 ...
 */
long np_pcg(const linearMatrix &Matrix_A, const linearMatrix &Matrix_M, const linearMatrix &Vector_B, linearMatrix &Vector_X,
						const float e, const int MaxIter)
{
	...
	while(myerror > new_e  && k < MaxIter)
	{
		...
		z = lAmatrixvecmult(Matrix_M, r);
		deltha = lAdotproduct(z, r);
		if(k == 1)
		{
			beta = 0;
			p = z;
